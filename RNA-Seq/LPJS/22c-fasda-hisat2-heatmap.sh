...
output_dir=Results/22-fasda-fc-hisat2
samples=$(ls -d $input_dir/chondro*.bam | wc -l)
...
for cell_type in chondro neuro; do
	...
	for end_time in $(seq $((start_time + 1)) 3); do
		...
		$output_dir/$cell_type-time$start_time-time$end_time-FC.txt \
		...
		$output_dir/$cell_type-time$start_time-norm.tsv \
		$output_dir/$cell_type-time$end_time-norm.tsv
	done
    done
done
